...
async def create_sellitem(_data: SellItems):
    _id = ser_insert_sellitem(_data, sellitem_collection, product_collection)
    return {"message": "Created successfully", "_id": _id}

@router.put("/sellitems/update")
def edit_sellitem(_data: SellItems):
    result = ser_update_sellitem(_data, sellitem_collection, product_collection)
    return result

@router.delete("/sellitems/delete/{sellitem_id}")
def remove_sellitem(sellitem_id: str):
    response = ser_delete_sellitem(sellitem_id, sellitem_collection, product_collection)
    return response